...
class HeadPoseTracker:
    ...
    # Индексы 6 ключевых точек Face Mesh (стандартные 468 landmarks)
    _LANDMARKS_IDS = [1, 33, 263, 61, 291, 199]

    # 3-D координаты тех же точек в пространстве головы
    ...
    def __init__(self):
        ...
        # Нейтральные углы (заполняются при калибровке)
        self.neutral_yaw: float = 0.0
        self.neutral_pitch: float = 0.0
        self.calibrated: bool = False

        # Пороги отклонения (из конфига)
        ...
        self._threshold_enter = 1.0   # множитель порога для входа
        self._threshold_exit = 0.6    # множитель порога для выхода

        # Буфер калибровки (накапливается во время калибровки глаз)
    ...
    def get_smooth_angles(self, frame: np.ndarray) -> Optional[Tuple[float, float]]:
        """Возвращает сглаженные (yaw, pitch) или None."""
    ...
    def accumulate_calibration(self, frame: np.ndarray) -> None:
        """Накапливаем показания во время калибровки глаз."""
        result = self.process_frame(frame)
    ...
    def finalize_calibration(self) -> bool:
        """Фиксируем нейтральное положение. Возвращает True при успехе."""
        if self._calib_yaw_buf and self._calib_pitch_buf:
            ...
            print(
                f"HeadPoseTracker: нейтраль зафиксирована. "
                f"Yaw={self.neutral_yaw:.1f}°, Pitch={self.neutral_pitch:.1f}°"
            )
            return True
...
class BehaviorAnalyzer:
    def __init__(self, max_suspicious_actions: int = 1):
        ...
        # Счётчики для головы (отдельный трекер последовательности)
        self._head_consecutive = 0
        self._head_min_consecutive = CONFIG.get("head_min_consecutive", 6)
        self._head_history: List[Dict] = []

        # Счётчики срабатываний по каналам (для поля cheating_trigger)
    ...
    def generate_report(self) -> Dict[str, any]:
        ...
        # Определяем причину срабатывания
...
class GUIInterface:
    ...
    def show_window2(self):
        ...
        self.progress_label = tk.Label(
            progress_frame, text="",
            font=("Arial", 36, "bold"), bg='black', fg='#00FF00'
        )
    ...
    def start_calibration(self):
        """Запуск калибровки (взгляд + голова одновременно)."""
        try:
        ...
        def calibration_loop():
            ...
            if remaining <= 0:
                if horizontal_values and vertical_values:
                    ...
                    print(
                        f"Калибровка завершена. Центр: "
                        f"H={self.main_app.gaze_tracker.horizontal_center:.2f}, "
                        f"V={self.main_app.gaze_tracker.vertical_center:.2f}"
                    )
                else:
                ...
                if frame is not None:
                    ...
                    try:
                        self.progress_label.config(
                            text=f"Калибровка... Осталось {int(remaining)} секунд. Не двигайте."
                        )
                    except:
                        pass
...
class SessionRecorder:
    ...
    def write_screen_frame(self, frame) -> None:
        self._write_with_timing(self.screen_writer, frame,
                                '_last_screen_time', '_screen_accum', '_screen_frame_idx')

    def write_webcam_frame(self, frame) -> None:
        self._write_with_timing(self.webcam_writer, frame,
                                '_last_webcam_time', '_webcam_accum', '_webcam_frame_idx')
...
class DataLogger:
    ...
    def log_head_data(self, head_info: Dict[str, any]) -> None:
        """Логирование данных о положении головы."""
        if head_info is None:
            return
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    ...
    def log_behavior(self, behavior_data: Dict[str, any]) -> None:
        """Логирование данных о поведении."""
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
